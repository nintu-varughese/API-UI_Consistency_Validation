...
test.describe.serial("Demoblaze Product Extraction and Comparison", () => {
  test("Extract product data from UI for Phones, Laptops, and Monitors", async ({
    demoblazeUI,
  }) => {
  ...
  test("Extract product data from API for Phones, Laptops, and Monitors", async ({
    demoblazeAPI,
  }) => {
  ...
  test("Compare API and UI JSON data", async ({
    compareJsonData,
    fs,
    path,
  }) => {
    const resultPath = "testsAssets/testData/compareResult.json";
    await test.step("Run JSON comparison", async () => {
      await compareJsonData();
      expect(
        fs.existsSync(resultPath),
        "Comparison result file not found"
      ).toBeTruthy();
    });
    ...
    await test.step("Validate comparison results", async () => {
      ...
      const allMatched = results.every(
        (r: any) => r.comparisonStatus === "Matched"
      );
      expect(
        allMatched,
        "Some mismatches detected in comparison result"
      ).toBeTruthy();
    });
  });
});
